...
| config_items              | array of object | Yes    | -             | Rate limiting rule items. The first matching `config_item` based on the order under `config_items` will trigger the rate limiting, and subsequent rules will be ignored |
...
Description of configuration fields for each item in `config_items`.
